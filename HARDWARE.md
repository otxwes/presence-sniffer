...
|---|---|---|---|---|
| 1 | Seeed XIAO ESP32-S3 ×3 — **plain non-Sense variant; headers optional** | $21 | edge sensors; **1 board → WiFi promiscuous, 1 → BLE scan, 1 → spare** | S3 is single-radio — cannot scan WiFi and BLE simultaneously, hence one board per radio. Header note: on-board USB-C works unsoldered; headers are *additive* (solder on later anytime, XIAO pads are forgiving castellated half-circles). If you have access to a soldering station and are comfortable with 0.5 mm-pitch castellated edges, unsoldered is fine and slightly cheaper + more flexible; pre-soldered remains a safe choice if you'd rather not solder. Core USB-only sensor path never depends on header joints |
