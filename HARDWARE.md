...
|---|---|---|---|---|
| 1 | Seeed XIAO ESP32-S3 ×3 — **pre-soldered (with headers), plain non-Sense variant** | $21 (+$8 vs unsoldered across 3 boards) | edge sensors; **1 board → WiFi promiscuous, 1 → BLE scan, 1 → spare** | S3 is single-radio — cannot scan WiFi and BLE simultaneously, hence one board per radio. Header decision (2026-09-19): going pre-soldered even with soldering station available — during prototyping, fewer assembly variables beats ~$8 savings; USB-only sensor path never depends on header joints, but headers avoid the 0.5 mm castellated soldering task while bench phase / enclosure building to focus debugging on firmware/hardware, not assembly. Header removal later is possible but mildly pad-stressing — revisit only if genuinely needed |
| 2 | Raspberry Pi 5 (**2GB or 4GB**; 1GB underspecced for aggregator) + official 27W PSU + official Active Cooler + 32GB+ A1 microSD — **bought piecemeal, NOT a starter-kit bundle** | $100–125 | aggregator host | Requires **5V/5A (25W)** PD supply, else USB budget caps at 600 mA. Vendor ranking (live-verified 2026-09-19): **Vilros cheapest** (board from $50.80, 27W PSU $14.46, Active Cooler $11.95, US free shipping) → get all three from Vilros in one order. Adafruit reliable but prices Pi 5 4GB at $130 / 8GB at $200 (markup over MSRP); PiShop 8GB $175; CanaKit starter kits ($149.95 1GB / $159.95 2GB / $204.95 4GB) bundle a case + 2× HDMI + card reader that our custom printed enclosure and headless deploy make worthless. Varied RAM same board performance — pick 2GB min, 4GB headroom |
